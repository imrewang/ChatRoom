...
void Client::Connect()
{
	...
	//创建socket
	sock = socket(AF_INET,SOCK_STREAM,0);//socket用于创建一个socket描述符，它唯一标识一个socket
	if(sock<0)
	{
		perror("socker error: ");
		exit(1);
	...
	//连接服务端
	if(connect(sock,(struct sockaddr*)&serverAddr,sizeof(serverAddr))<0)//connect()用于建立与指定 socket 的连接
	{
	...
	//创建epoll
	epfd = epoll_create(EPOLL_SIZE);//内核会产生一个epoll 实例数据结构并返回一个文件描述符，这个特殊的描述符就是epoll实例的句柄
...
void Client::Start()
{
	...
	}else
	{
		...
		while(isClientwork)
		{
			int epoll_events_count = epoll_wait(epfd,events,2,-1);//阻塞等待注册的事件发生，返回事件的数目，并将触发的事件写入events数组中。
			...
			for(int i = 0;i<epoll_events_count;i++)
			{
				...
				{
					//接收服务端广播消息
					int ret = recv(sock,recv_buf,BUF_SIZE,0);//recv函数用于socket通信中接收消息
					//清空结构体
